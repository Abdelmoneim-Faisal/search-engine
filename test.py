...
extendedStopWordsList.remove("in")
extendedStopWordsList.remove("to")
extendedStopWordsList.remove("where")
...
query_tokonization = [w.lower() for w in query_tokonization]
sW = set(stopwords.words('english'))
extendedStopWordsList.extend(sW)
extendedStopWordsList.remove("in")
extendedStopWordsList.remove("to")
extendedStopWordsList.remove("where")
query_tokonization = [w for w in query_tokonization if not w.lower() in extendedStopWordsList]
fileArray=[]
printCounter=0
...
for file in files:
    ...
    for term in positional_index:
        if file in positional_index[term]:
            ...
            vector_space_model[file][term] = [0, 0, 0, 0, 0]
            a1[file][term]=[0]
...
print("\n")
print("____________________________TF-Weight_______________________________________")            
print(pd.DataFrame(tfweight))
print("\n")
print("____________________________DF-IDF_______________________________________")            
for term in positional_index:
    print(term+", "+"DF : "+str(len(positional_index[term]))+" , "+
    "IDF : "+str(math.log10(len(files) /len(positional_index[term])))+"\n")
print("____________________________TF-IDF_______________________________________")            
print(pd.DataFrame(tfidf))
print("\n")
print("____________________________D-Length_______________________________________")            
tf_idf_square_summation=0
length={}
for file in files:
    for term in vector_space_model[file]:
        tf_idf_square_summation += math.pow(vector_space_model[file][term][3],2)
    length_of_document = math.sqrt(tf_idf_square_summation)
    length[file]=length_of_document
    print(file+" : "+str(length_of_document))
    tf_idf_square_summation=0
print("\n")
print("____________________________Normalized_TF-IDF	_______________________________________")   
normalize={}
for file in files:
    normalize[file]={}
    for term in vector_space_model[file]:
        normalization=(vector_space_model[file][term][3]/(length.get(file)))
        normalize[file][term]=normalization
print(pd.DataFrame(normalize))
print("\n")
